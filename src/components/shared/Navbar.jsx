...
const Navbar = () => {
    ...
    return (
        <nav className="w-full md:w-[85%] flex justify-between items-center my-5 p-3 bg-zinc mx-auto rounded-full">
            <h1 className='text-3xl text-white ml-5'>useCoursia</h1>  
                ...
                <ul className={`flex flex-col md:flex-row justify-between items-center gap-3 md:gap-10 absolute md:relative bg-zinc-900 md:bg-transparent p-4 md:p-0 rounded-lg md:rounded-none ${menuOpen ? 'top-16' : '-top-full'} transition-all duration-300`}>
                    {navList.map((item) => (
                        <li key={item} className="w-full md:w-auto">
                            <a href={`#${item.toLowerCase()}`} className="text-white text-xl cursor-pointer hover:text-blue transition duration-300 block text-center md:inline">
                                {item}
                            </a>
                        </li>
                    ))}
